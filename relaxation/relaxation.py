...
# directory from which simulations are started
backupDirectory = sys.argv[1]
# Specify directory in which results are saved. Script will generate subdirectories 
# specifying stimulation parameters
outputParentDirectory=sys.argv[3]
# total simulation time in seconds 
SimulationTime=int(sys.argv[2])

# create a complete backup at the following times
TBackupSteps = np.arange(6000, 100100, 200) # sec
...
outputFolder= outputParentDirectory
...
Tmax=SimulationTime + 0.001*(kStepInit + kSave)*dt  # sec
...
while kStep < nSteps:
	...
	if numberOfSpikingNeurons>0:
		...
		switchOffSpikeTimes[passedThreshold]=(t+tau_spike)*np.ones(numberOfSpikingNeurons)
		...
		if STDPAlreadyOn==True:
			...
			# generate scipy.sparse matrix containing weight updates for respective synapse
			weigthUpdatesPre=scipy.sparse.csc_matrix((weigthUpdates, (cMatrixIndizes)), shape=(N, N))


			# update coupling matrix
			cMatrix+= weigthUpdatesPre #-  cMInh.multiply(weigthUpdatesPre)
