...
#
#       input:   outputFolder
#           outputFolder  .. path to parent directory
#       output: outputFolder
#             outputFolder ...  outputDirectory + '/STDP_on/seed+'+str(initialSeed)
def create_outputFolder( outputFolder ):
...
def initialize_system( system_parameters ):
    ...
    ### new version: this accounts for a factor of tauSyn in the equations for the synaptic conductances
    # excitatory weights
    basicFilterSpikingNeurons[:N_STN,0]=cMaxExc*np.ones(N_STN)
    # inhibitory weights
    basicFilterSpikingNeurons[N_STN:,1]=cMaxInh*np.ones(N_GPe)
